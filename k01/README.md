...
## ソースコードの説明
68.変数を定義する
69.変数を定義し、見つからなかった時のために-１を代入
71~76.配列の位置となるposがsizeより小さい場合は繰り返し処理を行う。一致しなかったら、posを１増やし、次のidと一致するか調べる。keyがidと一致したら、resultをposにしてfor文から抜けだす。見つからなかったら、resultはそのまま
77.returnの値を返す

83.変数を定義する
84.変数を定義し、見つからなかった時のために-１を代入
85.左の値が右の値以下ならwhile文を繰り返す
86.中央の値をposに代入する
87~89.keyがidと一致したらresultにposを代入し、while文から抜け出す
90,91.keyがidより小さいとき、leftをposより一つ大きい数にする
92,93.keyがidより大きいとき、rightをposより一つ小さい数にする
97.resultを返す
